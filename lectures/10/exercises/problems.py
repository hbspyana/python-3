...
def threaded_square_map(values: list[int]) -> list[int]:
    ...
    threads = []
    for v in values:
        t = threading.Thread(target=square, args=(v,))
        threads.append(t)
        t.start()
    for t in threads:
        t.join()
...
def threadpool_sleep_map(delays: list[float], max_workers: int = 4) -> list[float]:
    ...
    """
    if max_workers < 1:
        raise ValueError('max_workers should be greater than 1')
    def task(delay: float):
        return simulated_long_fetch(delay)
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        outputs = list(executor.map(task, delays))
    return outputs
...
def processpool_square_map(values: list[int], max_workers: int = 2) -> list[int]:
    ...
    with ProcessPoolExecutor as executor:
        outputs = list(executor.map(square, values))
    return outputs
...
async def async_tag_fetch(labels: list[str], delay: float = 0.01) -> list[str]:
    ...
    async def task(label: str):
        ...
        return f"done:{label}"
    tasks = [task(label) for label in labels]
